...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!text || text.trim().length < 50) {
      return NextResponse.json(
        { error: "Document content unreadable or too short." },
        { status: 400 },
      );
    }
    ...
    if (docError)
      throw new Error(`Document DB entry failed: ${docError.message}`);
    ...
    const chunkRows = chunks.map((chunk, index) => {
      ...
      const nextChunkId =
        index < chunks.length - 1 ? generatedIds[index + 1] : null;
    ...
    console.log(
      `🔗 [GraphRAG Ingestion] Tissage du graphe linéaire pour ${chunks.length} chunks. ID Document: ${document.id}`,
    );
    ...
    if (chunkError)
      throw new Error(
        `Vector chunks database injection failed: ${chunkError.message}`,
      );
